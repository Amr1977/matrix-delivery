...
const validEnvs = ["development", "test"];
if (!validEnvs.includes(env)) {
  console.log(`❌ Invalid environment: ${env}. Use: development or test`);
  console.log("Usage: node start-dev.js --env=development");
  console.log("       node start-dev.js --env=test");
  process.exit(1);
}
...
function startBackend() {
  return new Promise((resolve, reject) => {
    ...
    // Copy environment file based on env
    const envSrc = path.join(process.cwd(), "backend", `.env.${env}`);
    const envDest = path.join(process.cwd(), "backend", ".env");
    ...
    try {
      if (fs.existsSync(envSrc)) {
        ...
        log(
          colors.blue,
          `📋 Copied ${env} environment configuration for backend`,
        );
      }
    } catch (error) {
    ...
    backendProcess.on("close", (code) => {
      if (!backendReady) {
        log(
          colors.red,
          `❌ Backend server failed to start (exit code: ${code})`,
        );
...
function startFrontend() {
  return new Promise((resolve, reject) => {
    ...
    const envSrc = path.join(
      process.cwd(),
      "frontend",
      `.env.${env === "test" ? "testing" : env === "development" ? "develop" : env}`,
    );
    ...
    try {
      if (fs.existsSync(envSrc)) {
        ...
        log(
          colors.blue,
          `📋 Copied ${env} environment configuration for frontend`,
        );
      }
    } catch (error) {
      log(colors.yellow, `⚠️  Could not copy environment file:`, error.message);
    ...
    frontendProcess.stdout.on("data", (data) => {
      ...
      if (
        (output.includes("Local:") || output.includes("localhost:3000")) &&
        !frontendReady
      ) {
        frontendReady = true;
        log(
    ...
    frontendProcess.on("close", (code) => {
      if (!frontendReady && code !== 0) {
        log(
          colors.red,
          `❌ Frontend server failed to start (exit code: ${code})`,
        );
    ...
    exec(
      ...
      (error, stdout, stderr) => {
        if (error) {
          log(
            colors.yellow,
            "⚠️  Backend health check failed (this is normal for development)",
          );
          resolve(false);
        } else {
...
async function main() {
  try {
    ...
    console.log("");
    log(colors.magenta, `⚠️  Environment: ${env.toUpperCase()}`);
    console.log("");
