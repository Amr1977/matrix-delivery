...
describe("DeliveryFSM", () => {
  ...
  describe("Initial State", () => {
    test("should have correct initial state", () => {
      expect(deliveryFSM.getInitialState()).toBe(
  ...
  describe("State Transitions", () => {
    ...
    test("should allow pickup transition", async () => {
      const context = {
        ...
        order: {
          id: 123,
          vendor_state: "order_is_fully_prepared_and_ready_for_delivery",
        },
        courier: { id: 1 },
    ...
    test("should allow customer confirmation transition", async () => {
      const context = {
        ...
        order: {
          id: 123,
          delivery_state: "awaiting_customer_confirmation_of_order_delivery",
        },
    ...
    test("should allow customer dispute transition", async () => {
      const context = {
        ...
        metadata: {
          ...
          dispute_details: "Ordered pizza, got burger",
        },
      };
  ...
  describe("Delivery Tracking Info", () => {
    test("should provide tracking information for initial state", () => {
      ...
      expect(trackingInfo.currentState).toBe(
        "delivery_request_created_waiting_for_courier_acceptance",
      );
      expect(trackingInfo.description).toBe(
        "Delivery request created, waiting for courier",
      );
    ...
    test("should provide tracking information for assigned state", () => {
      ...
      expect(trackingInfo.description).toBe(
        "Courier assigned to deliver your order",
      );
      expect(trackingInfo.isTerminal).toBe(false);
  ...
  describe("Possible Events", () => {
    test("should return possible events for initial state", () => {
      const events = deliveryFSM.getPossibleEvents(
        "delivery_request_created_waiting_for_courier_acceptance",
      );
    ...
    test("should return empty array for terminal states", () => {
      const events1 = deliveryFSM.getPossibleEvents(
        "order_delivery_successfully_completed_and_confirmed_by_customer",
      );
      const events2 = deliveryFSM.getPossibleEvents(
        "delivery_disputed_by_customer_and_requires_resolution",
      );
