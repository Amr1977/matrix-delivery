...
describe("PaymentFSM", () => {
  ...
  describe("Terminal States", () => {
    test("should identify terminal states correctly", () => {
      expect(paymentFSM.isTerminalState("payment_pending_for_customer")).toBe(
        false,
      );
      expect(
        paymentFSM.isTerminalState(
          "payment_successfully_received_and_verified_for_order",
        ),
      ).toBe(true);
      expect(
        paymentFSM.isTerminalState("payment_has_been_refunded_to_customer"),
      ).toBe(true);
      expect(
        paymentFSM.isTerminalState("payment_attempt_failed_for_order"),
      ).toBe(true);
    });
  });
  ...
  describe("Possible Events", () => {
    test("should return possible events for pending payment state", () => {
      const events = paymentFSM.getPossibleEvents(
        "payment_pending_for_customer",
      );
