...
describe('Payment Configuration Tests', function () {
    ...
    describe('Configuration Constants', function () {
        it('should have correct commission rate', function () {
            expect(PAYMENT_CONFIG.COMMISSION_RATE).to.equal(0.15);
        });

        it('should calculate percentage from decimal', function () {
            expect(PAYMENT_CONFIG.COMMISSION_RATE_PERCENT).to.equal(15);
        });

        it('should calculate basis points from decimal', function () {
            expect(PAYMENT_CONFIG.COMMISSION_RATE_BASIS_POINTS).to.equal(1500);
        });
        ...
        it('should support multiple currencies', function () {
            expect(PAYMENT_CONFIG.SUPPORTED_CURRENCIES).to.include.members(['EGP', 'USD', 'EUR']);
        });
    });
    ...
    describe('Commission Calculation', function () {
        ...
        it('should handle large amounts', function () {
            ...
            expect(commission).to.equal(1500.00);
            expect(payout).to.equal(8500.00);
        });

        it('should handle small amounts', function () {
            const { commission, payout } = calculateCommission(10);
            expect(commission).to.equal(1.50);
            expect(payout).to.equal(8.50);
        });

        it('should support custom commission rates', function () {
            const { commission, payout, rate } = calculateCommission(100, 0.10);
            expect(commission).to.equal(10.00);
            expect(payout).to.equal(90.00);
            expect(rate).to.equal(0.10);
        });
    });
    ...
    describe('Payment Fee Calculation', function () {
        ...
        it('should handle different amounts', function () {
            expect(calculatePaymentFee(200, 'card')).to.equal(5.00);
            expect(calculatePaymentFee(500, 'wallet')).to.equal(12.50);
        });
    });
    ...
    describe('Payment Methods Configuration', function () {
        ...
        it('should have crypto supported tokens', function () {
            expect(PAYMENT_CONFIG.PAYMENT_METHODS.CRYPTO.supportedTokens).to.include.members(['USDC', 'USDT']);
        });
    });
    ...
    describe('Edge Cases', function () {
        ...
        it('should handle very large commission', function () {
            ...
            expect(commission).to.equal(150000.00);
        });
