...
describe('OfferService - Unit Tests', () => {
  ...
  describe('createOffer', () => {
    it('should create a percentage discount offer successfully', async () => {
      ...
      pool.query.mockResolvedValueOnce({
        rows: [{ vendor_id: mockVendorId, price: 50.00 }]
      });
    ...
    it('should throw error for missing required fields', async () => {
      ...
      await expect(offerService.createOffer(mockVendorId, invalidPayload))
    ...
    it('should throw error for fixed discount exceeding item price', async () => {
      ...
      await expect(offerService.createOffer(mockVendorId, invalidPayload))
        .rejects.toThrow('Fixed discount cannot be greater than or equal to item price');
    });
    ...
    it('should throw error when item not found', async () => {
      ...
      await expect(offerService.createOffer(mockVendorId, mockOfferData))
    ...
    it('should throw error for conflicting offer dates', async () => {
      ...
      offerRepository.getActiveOffersByItem.mockResolvedValue([conflictingOffer]);

      await expect(offerService.createOffer(mockVendorId, mockOfferData))
        .rejects.toThrow('Item already has an active offer during the specified date range');
    });
  });
  ...
  describe('getOfferById', () => {
    ...
    it('should throw error when offer not found', async () => {
      ...
      await expect(offerService.getOfferById(mockOfferId, mockVendorId))
  ...
  describe('updateOffer', () => {
    it('should update offer successfully', async () => {
      ...
      offerRepository.getOfferById.mockResolvedValue(mockOffer);
      offerRepository.updateOffer.mockResolvedValue({ ...mockOffer, ...updateData });

      const result = await offerService.updateOffer(mockOfferId, mockVendorId, updateData);

      expect(result.title).toBe('Updated Title');
      expect(result.discount_value).toBe(25);
      expect(offerRepository.updateOffer).toHaveBeenCalledWith(mockOfferId, updateData);
    });
    ...
    it('should validate updated discount value', async () => {
      ...
      await expect(offerService.updateOffer(mockOfferId, mockVendorId, invalidUpdate))
        .rejects.toThrow('Discount value must be greater than 0');
    });
  ...
  describe('deactivateOffer', () => {
    it('should deactivate offer successfully', async () => {
      ...
      offerRepository.getOfferById.mockResolvedValue(mockOffer);
      offerRepository.updateOffer.mockResolvedValue({ ...mockOffer, status: false });

      const result = await offerService.deactivateOffer(mockOfferId, mockVendorId);

      expect(result.status).toBe(false);
      expect(offerRepository.updateOffer).toHaveBeenCalledWith(mockOfferId, { status: false });
    });
  });
  ...
  describe('getActiveOffersByItem', () => {
    it('should return active offers for an item', async () => {
      ...
      expect(offerRepository.getActiveOffersByItem).toHaveBeenCalledWith(mockItemId);
    });
  });
  ...
  describe('getOffersByItem', () => {
    it('should return offers for vendor-owned item', async () => {
      ...
      const result = await offerService.getOffersByItem(mockItemId, mockVendorId);

      expect(result).toEqual(offers);
      expect(offerRepository.getOffersByItem).toHaveBeenCalledWith(mockItemId, 50, 0);
    });
    ...
    it('should throw error for non-owned item', async () => {
      ...
      await expect(offerService.getOffersByItem(mockItemId, mockVendorId))
        .rejects.toThrow('Access denied: Item does not belong to your store');
    });
  });
  ...
  describe('calculateDiscountedPrice', () => {
    it('should calculate percentage discount correctly', () => {
      const offer = { ...mockOffer, discount_type: 'percentage', discount_value: 20 };
      const originalPrice = 100;

      const result = offerService.calculateDiscountedPrice(originalPrice, offer);

      expect(result).toBe(80); // 100 - (100 * 0.20)
    });

    it('should calculate fixed discount correctly', () => {
      const offer = { ...mockOffer, discount_type: 'fixed', discount_value: 15 };
      const originalPrice = 100;
    ...
    it('should return original price for inactive offer', () => {
      ...
      const result = offerService.calculateDiscountedPrice(originalPrice, inactiveOffer);
    ...
    it('should not allow negative prices', () => {
      const offer = { ...mockOffer, discount_type: 'fixed', discount_value: 150 };
      const originalPrice = 100;
