...
When(
  "{string} publishes an order for {string} priced at {string}",
  async function (user, title, price) {
    await this.adapter.publishOrder(user, title, price);
  },
);

Then(
  "the order {string} should be available in the marketplace",
...
When(
  "{string} places a bid of {string} on order {string}",
  async function (driver, amount, orderTitle) {
    await this.adapter.driverBidsOnOrder(
      orderTitle,
      parseFloat(amount),
      driver,
    );
...
Then(
  "{string} should see a bid of {string} from {string}",
  async function (customer, amount, driver) {
    ...
    if (this.adapter.checkBidExists) {
      const hasBid = await this.adapter.checkBidExists(
        customer,
        driver,
        parseFloat(amount),
      );
...
When(
  "{string} accepts the bid from {string}",
  async function (customer, driver) {
...
Then(
  "{string} should see the order in their {string} list",
  async function (user, listType) {
...
Then(
  "{string} wallet should be credited with {string} less commission",
  async function (user, amount) {
...
When(
  "{string} reviews {string} with {string} stars and comment {string}",
  async function (reviewer, reviewee, rating, comment) {
    if (this.adapter.submitReview) {
      await this.adapter.submitReview(
        reviewer,
        reviewee,
        parseInt(rating, 10),
        comment,
