...
const topupRateLimit = createLimiter(
  {
    ...
    limit: 10,
    message:
      "Too many top-up requests. Please wait a moment before trying again.",
    keyGenerator: (req) => {
...
// Route Calculation Rate Limit
// 60 requests per minute (1 per second - reasonable for map interactions)
const routeCalculationRateLimit = createLimiter(
  {
    windowMs: 60 * 1000, // 1 minute
    limit: 60,
    message: "Too many route calculations, please wait a moment",
    keyGenerator: (req) => {
      return req.headers["x-device-fingerprint"] || ipKeyFromRequest(req);
    },
  },
...
module.exports.routeCalculationRateLimit = routeCalculationRateLimit;
