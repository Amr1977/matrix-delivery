  ...
  io.on("connection", (socket) => {
    ...
    socket.on("join_order", async (data) => {
      ...
      try {
        ...
        if (
          order.customer_id !== socket.userId &&
          order.assigned_driver_user_id !== socket.userId
        ) {
    ...
    socket.on("update_location", async (data) => {
      ...
      try {
        ...
        if (
          ...
          orderResult.rows[0].assigned_driver_user_id !== socket.userId
        ) {
          socket.emit("error", { message: "Unauthorized" });
          return;
        }
        ...
        console.log(
          `📍 [Socket] Emitted location_update for order ${orderId}:`,
          latitude,
          longitude,
        );
      } catch (error) {
