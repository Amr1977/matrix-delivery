...
const {
  createNotification,
} = require("../services/notificationService"); /* P0 FIX: removed .ts ext */ // Import from notification service
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to start tracking" });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to stop tracking" });
    }
  },
);
...
router.get("/tracking/:orderId/status", verifyToken, async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Unauthorized to view this order tracking" });
    }
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to get tracking status" });
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (typeof isOnline !== "boolean") {
        return res
          .status(400)
          .json({ error: "isOnline must be a boolean value" });
      }
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to update driver status" });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      const { orderId } = req.params;
      const { latitude, longitude, heading, speedKmh, accuracyMeters } =
        req.body;
      const driverId = req.user.userId;
      ...
      if (order.current_tracking_status !== "in_progress") {
        return res
          .status(400)
          .json({ error: "Tracking is not active for this order" });
      }
      ...
      try {
        ...
        logger.warn(
          `Socket emit error (non-critical): ${socketError.message}`,
          {
            orderId,
            category: "socket",
          },
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get location" });
    }
  },
);
...
router.get(
  "/location/order/:orderId/bidders",
  verifyToken,
  async (req, res) => {
    try {
      const { orderId } = req.params;
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get locations" });
    }
  },
);
    ...
    try {
      ...
      const getEarningsSum = async (startDateParam, endDateParam = null) => {
        ...
        if (endDateParam) {
         ...
         AND status IN ('delivered', 'courier_delivered', 'customer_delivered')
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get earnings stats" });
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to get earnings history" });
    }
  },
);
