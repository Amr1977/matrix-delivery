...
const NOMINATIM_REVERSE_URL_BASE =
...
const LiveTrackingMap = ({
  ...
  const validOrderId =
    typeof orderId === "string" && orderId ? orderId : String(orderId || "");
  ...
  const fetchTrackingData = useCallback(async () => {
    try {
      ...
      if (!orderMeta) {
        try {
          ...
          const match = Array.isArray(ordersList)
            ? ordersList.find(
                (o) => o.id === validOrderId || o.orderNumber === validOrderId,
              )
            : null;
          if (match) {
      ...
      try {
        ...
        console.warn(
          "Primary tracking endpoint failed, trying alternative:",
          primaryErr.message,
        );
        ...
        try {
          ...
          console.error(
            "Fallback order details also failed:",
            fallbackErr.message,
          );
  ...
  useEffect(() => {
    if (validOrderId) {
      ...
      socket.on("location_update", (data) => {
        console.log(
          "📡 LiveTrackingMap: Real-time location update received",
          data,
        );
      ...
      refreshIntervalRef.current = setInterval(
        fetchTrackingData,
        LIVE_TRACK_REFRESH_INTERVAL_MS_IDLE,
      );
  ...
  useEffect(() => {
    ...
    const getInterval = () => {
      ...
      return isActive
        ? LIVE_TRACK_REFRESH_INTERVAL_MS_ACTIVE
        : LIVE_TRACK_REFRESH_INTERVAL_MS_IDLE;
    };
    ...
    const handleVisibilityChange = () => {
      ...
      console.log(
        `🔍 Live tracking interval adjusted: ${document.hidden ? "HIDDEN (5min)" : isActive ? "ACTIVE (15s)" : "IDLE (60s)"}`,
      );
    };
  ...
  useEffect(() => {
    ...
    const handlePositionUpdate = (position) => {
      ...
      const timeElapsed =
        now - last.timestamp > SMART_TRACKING_TIME_THRESHOLD_MS;
  ...
  const getCenter = () => {
    if (trackingData?.currentLocation) {
      ...
      trackingData?.pickup?.location &&
      trackingData?.delivery?.location
    ) {
      return [
        (trackingData.pickup.location.lat +
          trackingData.delivery.location.lat) /
          2,
        (trackingData.pickup.location.lng +
          trackingData.delivery.location.lng) /
          2,
      ];
    }
  ...
  const bounds = React.useMemo(() => {
    ...
    if (
      ...
      typeof trackingData.currentLocation.lat === "number"
    ) {
      points.push([
        trackingData.currentLocation.lat,
        trackingData.currentLocation.lng,
    ...
    if (
      ...
      Array.isArray(trackingData.locationHistory)
    ) {
  ...
  const mapContent = (
    ...
    >
      ...
      >
        ...
        {(() => {
          const nextType =
            trackingData.status === "accepted" ? "pickup" : "delivery";
          const nextLoc =
            nextType === "pickup"
              ? trackingData.pickup?.location
              : trackingData.delivery?.location;
          ...
          const distanceKm =
            cur && nextLoc
              ? haversineKm(
                  ...
                  { lat: nextLoc.lat, lng: nextLoc.lng },
                )
              : null;
      ...
      >
        <div
          style={{
            flex: 1,
            padding: "0.75rem",
            border: `2px solid ${!["delivered", "courier_delivered", "customer_delivered", "completed"].includes(trackingData.status) ? "#10B981" : "#34D399"}`,
            borderRadius: "0.375rem",
        ...
        <div
          style={{
            flex: 1,
            padding: "0.75rem",
            border: `2px solid ${["delivered", "courier_delivered", "customer_delivered", "completed"].includes(trackingData.status) ? "#34D399" : "#6B7280"}`,
            borderRadius: "0.375rem",
            background: [
              "delivered",
              "courier_delivered",
        ...
        >
          ...
          {expectedToPickup.length === 2 && (
            <Polyline
              ...
              pathOptions={{
                className: "expected-route-pickup animated-stroke",
              }}
            />
          )}
          ...
          {expectedToDelivery.length === 2 && (
            <Polyline
              ...
              dashArray={POLYLINE_DASH_PATTERN}
              pathOptions={{
                className: "expected-route-delivery animated-stroke",
              }}
            />
          )}
            ...
            >
              <Popup>
                ...
                {trackingData.pickup.completedAt && (
                  <small>
                    Completed:{" "}
                    {new Date(trackingData.pickup.completedAt).toLocaleString()}
                  </small>
                )}
              </Popup>
            </Marker>
          ...
          {trackingData.currentLocation && (
            ...
            >
              <Popup>
                ...
                <br />
                Driver: {safeRender(trackingData.driver?.name)}
                <br />
                Speed:{" "}
                {trackingData.currentLocation.speedKmh
                  ? `${trackingData.currentLocation.speedKmh} km/h`
                  : "Unknown"}
                <br />
                Last Update:{" "}
                {new Date(
                ...
                <br />
                Accuracy:{" "}
                {safeRender(trackingData.currentLocation.accuracyMeters)}m<br />
      ...
      >
        ...
        <div>
          ...
          {["accepted", "picked_up", "in_transit"].includes(trackingData.status)
            ? `${LIVE_TRACK_REFRESH_INTERVAL_MS_ACTIVE / 1000} seconds`
            : `${LIVE_TRACK_REFRESH_INTERVAL_MS_IDLE / 1000} seconds`}
        </div>
      </div>
    </div>
  );
