...
const OrderCard = ({
  ...
  const { t } = useI18n();
  const [showRouteMapFullscreen, setShowRouteMapFullscreen] =
  ...
  const { locations: bidLocations, loading: loadingLocations } =
    useBidsLocations(
      order.id,
    ...
    >
        ...
        currentUser?.primary_role === "customer" && (
          ...
          >
            {(() => {
              ...
              window.console.log(
                "ðŸ“¦ [CUSTOMER PENDING] OrderCard - Order data:",
                debugInfo,
              );
              return null;
            })()}
            <RoutePreviewMap
              ...
              bids={
                bidLocations && bidLocations.length > 0
                  ? bidLocations
                  : order.bids || []
              }
              compact={true}
      ...
      >
        ...
        >
          ...
          <div>
            ...
            >
              {formatAddress(order.pickupAddress || order.from?.name)}
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div>
            ...
            >
              {formatAddress(order.deliveryAddress || order.to?.name)}
            </p>
          </div>
        </div>
      </div>
      ...
      {order.assignedDriver && (
        ...
        >
          ...
          >
            ...
            >
              {order.assignedDriver.profilePicture ? (
                <img
                  src={order.assignedDriver.profilePicture}
                  alt="Driver"
                  style={{
                    width: "100%",
                    height: "100%",
            ...
            <div>
              ...
              >
                {order.assignedDriver.name || "Unknown Driver"}
              </p>
              <div
                style={{
                  display: "flex",
              ...
              >
                <span>
                  â­ {order.assignedDriver.rating?.toFixed(1) || "New"}
                </span>
                <span>â€¢</span>
                <span>
                  {order.assignedDriver.totalDeliveries || 0} deliveries
                </span>
              </div>
            </div>
          </div>
          ...
          <div
            style={{ display: "grid", gap: "0.75rem", fontSize: "0.875rem" }}
          >
      ...
      >
        ...
        <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap" }}>
          {[
            ...
            <>
              {currentUser?.primary_role === "customer" &&
                !order.reviewStatus?.reviews.toDriver && (
                  <button
                    onClick={() =>
                      onOpenReviewModal(order.id, "customer_to_driver")
                    }
              ...
              {currentUser?.primary_role === "driver" &&
                ...
                !order.reviewStatus?.reviews.toCustomer && (
                  <button
                    onClick={() =>
                      onOpenReviewModal(order.id, "driver_to_customer")
                    }
                    className="btn-success"
              ...
              {!order.reviewStatus?.reviews.toPlatform && (
                <button
                  onClick={() =>
                    onOpenReviewModal(
                      order.id,
                      `${currentUser?.primary_role}_to_platform`,
                    )
                  }
            ...
            typeof onDeleteOrder === "function" && (
              <button
                ...
                style={{
                  ...
                  opacity: loadingStates?.deleteOrder ? 0.5 : 1,
                }}
              >
                ðŸ—‘ï¸ Cancel Order
              </button>
        ...
        currentUser?.primary_role === "driver" && (
          ...
          >
                ...
                >
                  ...
                  {!order.customerIsVerified && (
                    <button
                      onClick={() =>
                        window.open(
                          `https://wa.me/${process.env.REACT_APP_WHATSAPP_ADMIN_NUMBER}?text=${encodeURIComponent(`Hello, I would like to verify my account for order ${order.orderNumber || order.id}. My user ID is: ${currentUser?.id}`)}`,
                          "_blank",
                        )
                      }
                      style={{
                        background:
                          "linear-gradient(135deg, #25D366 0%, #10B981 50%, #25D366 100%)",
                        color: "#FFFFFF",
            ...
            {order.distance && (
              ...
              >
                ðŸ“ Distance from pickup:{" "}
                {order.distance ? `${order.distance.toFixed(2)} km` : "Unknown"}
              </div>
            )}
            ...
            <DriverBiddingMap
              ...
              onToggleFullscreen={() =>
                setShowRouteMapFullscreen((prev) => !prev)
              }
            ...
            >
              ...
              <button
                ...
                style={{
                  padding: "0.5rem 1rem",
                  background:
                    "linear-gradient(135deg, #4F46E5 0%, #8B5CF6 50%, #4F46E5 100%)",
                  color: "#FFFFFF",
        ...
        currentUser?.primary_role === "customer" && (
          ...
          >
            <RoutePreviewMap
              ...
              dropoff={
                order.to ||
                (order.dropoffLocation && order.dropoffLocation.coordinates)
              }
          ...
          >
            ...
            >
              {order.bids.map((bid, index) => {
                const isHighlighted =
                  highlightedBidId === (bid.userId || bid.driver_id);
                return (
                  <div
                    key={index}
                    ...
                    onMouseEnter={() =>
                      setHighlightedBidId(bid.userId || bid.driver_id)
                    }
          ...
          >
            ...
            >
              ...
              >
                ...
                {order.assignedDriver?.name || "Assigned Driver"}
              </p>
              <p
                style={{
                  fontSize: "0.875rem",
