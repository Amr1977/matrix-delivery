...
interface ProfilePageProps {
    ...
    API_URL: string;
    token: string | null;
    setProfileData: (data: User | ((prev: User) => User)) => void;
    setCurrentUser: ((user: User) => void) | null;
    currentUser: User | null;
    optimizeAndUploadProfilePicture: ((file: File) => void) | null;
    ...
    favorites: Favorite[];
    setFavorites: (favorites: Favorite[] | ((prev: Favorite[]) => Favorite[])) => void;
    onNavigate: ((view: string) => void) | null;
}
...
const ProfilePage: React.FC<ProfilePageProps> = ({
    ...
    const updateProfile = async (patch: Partial<User>) => {
        ...
        try {
            ...
            const { id, email, created_at, updated_at, role, primary_role, granted_roles, ...updatableFields } = patch as any;
            ...
            setProfileData((prev: User) => ({ ...prev, ...d.user }));
    ...
    const removePaymentMethod = async (id: string) => {
        if (!token) return;
        try {
            const res = await fetch(`${API_URL}/users/me/payment-methods/${id}`, {
                method: 'DELETE',
    ...
    const handleRoleSwitch = (updatedUser: User) => {
        ...
        <div>
            ...
            <div style={{ marginTop: '10px', display: 'flex', gap: '10px' }}>
                <span style={{
                    padding: '4px 12px',
                    borderRadius: '20px',
                    background: (profileData.is_verified || profileData.isVerified) ? 'rgba(16, 185, 129, 0.2)' : 'rgba(255, 255, 255, 0.1)',
                    color: (profileData.is_verified || profileData.isVerified) ? '#10B981' : '#9CA3AF',
                    fontSize: '14px',
                    fontWeight: '500'
                }}>
                    {(profileData.is_verified || profileData.isVerified) ? '✓ Verified' : 'Unverified'}
                </span>
                <span style={{
                    padding: '4px 12px',
                    borderRadius: '20px',
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '10px' }}>
        ...
        {(profileData.granted_roles?.length && profileData.granted_roles.length > 1 || profileData.roles?.length && profileData.roles.length > 1) && (
    ...
    {/* Driver Specifics */ }
{
    (profileData.primary_role === 'driver' || profileData.role === 'driver' || (profileData.roles && profileData.roles.includes('driver')) || (profileData.granted_roles && profileData.granted_roles.includes('driver'))) && (
        <div className=\"card\" style={{ padding: '20px', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
            < h3 style = {{ marginTop: 0, color: '#A7F3D0', borderBottom: '1px solid rgba(255,255,255,0.05)', paddingBottom: '10px' }
}>🚗 Driver Settings</h3 >
    ...
    <div style={{ padding: '15px', background: 'rgba(0,0,0,0.2)', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        onChange={async (e) => {
            try {
                const res = await fetch(`${API_URL}/users/me/availability`, {
                    method: 'POST',
                ...
                if (res.ok) {
                    const d = await res.json();
                    setProfileData((prev: User) => ({ ...prev, is_available: d.isAvailable }));
                }
            } catch (err) {
                if (setError) setError(err instanceof Error ? err.message : String(err));
            }
