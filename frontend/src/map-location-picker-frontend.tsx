          ...
          >
            <input
              ...
              onChange={(e) =>
                setAgentPreferences({
                  ...agentPreferences,
                  accept_remote_areas: e.target.checked,
                })
              }
            />
            <span style={{ fontSize: "0.875rem", fontWeight: "600" }}>
          ...
          >
            <input
              ...
              onChange={(e) =>
                setAgentPreferences({
                  ...agentPreferences,
                  accept_international: e.target.checked,
                })
              }
            />
            <span style={{ fontSize: "0.875rem", fontWeight: "600" }}>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: "1.25rem", fontWeight: "bold" }}>
                {pickupLocation.isRemote || dropoffLocation.isRemote
                  ? "⚠️ Yes"
                  : "✅ No"}
              </p>
            </div>
  ...
  const handleMapClick = async (coords) => {
    ...
    try {
      const data = await api.get(
        `/locations/reverse-geocode?lat=${coords.lat}&lng=${coords.lng}`,
      );
      onChange(data);
    } catch (err) {
      setError(err.message);
...
const RoutePreviewMap = ({ pickup, dropoff, routeInfo, loading }) => {
  ...
  const routePath = routeInfo?.polyline
    ? []
    : [
      ...
      {routeInfo && (
        <>
            ...
            >
              ...
              <div>
                ...
                <p style={{ fontSize: "0.875rem", fontWeight: "600" }}>
                  {routeInfo.route_found
                    ? "✅ Optimized Route"
                    : "⚠️ Straight Line Estimate"}
                </p>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            >
              {Object.entries(routeInfo.estimates || {}).map(
                ([vehicle, data]) => (
                  <div
                    key={vehicle}
                    style={{
