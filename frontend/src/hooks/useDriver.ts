...
const useDriver = (token: string | null, currentUser: User | null) => {
  const [viewType, setViewType] = useState<"active" | "bidding" | "history">(
    "active",
  );
  ...
  const [locationPermission, setLocationPermission] = useState<
    "unknown" | "granted" | "denied" | "unavailable" | "timeout"
  >("unknown");
  ...
  const API_URL =
    process.env.REACT_APP_API_URL || "https://matrix-api.oldantique50.com/api";
  ...
  const getFilteredDriverOrders = useCallback((): Order[] => {
    ...
    switch (viewType) {
      ...
      case "bidding":
        filteredOrders = orders.filter(
          (order) => order.status === "pending_bids" && !order.assignedDriver,
        );
  ...
  const getAreasFromOrders = useCallback(
    (country: string = "", city: string = ""): string[] => {
      ...
      orders.forEach((order) => {
        if (order.status === "pending_bids") {
          ...
          if (
            (!country || pickupParts.country === country) &&
            (!city || pickupParts.city === city)
          ) {
            if (pickupParts.area) areas.add(pickupParts.area);
          }
          if (
            (!country || deliveryParts.country === country) &&
  ...
  const reverseGeocodeCurrentLocation = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/locations/reverse-geocode?lat=${driverLocation.latitude}&lng=${driverLocation.longitude}`,
      );
