...
development: {
  apiKey: "YOUR_DEV_API_KEY", // Replace with your Firebase API key
...
STRIPE_SECRET_KEY=sk_test_your_key_here  // Replace with your Stripe test key

// EXPOSED: Database credentials with weak defaults
DB_PASSWORD=your_secure_password_here  // Use strong, unique passwords
JWT_SECRET=your_unique_jwt_secret_here  // Generate: node -e "console.log(require('crypto').randomBytes(32).toString('hex'))"
```
...
// backend/.env
JWT_SECRET = your_jwt_secret_here; // Generate unique strong secret
// backend/.env.production
JWT_SECRET = your_production_secret_here; // Generate different strong secret
...
app.get(
  "/api/browse/vendors-near",
  rateLimit(200, 60 * 1000),
  async (req, res) => {
  ...
  ssl:
    process.env.NODE_ENV === "production"
      ? {
          rejectUnauthorized: true,
          ca: fs.readFileSync(
            process.env.DB_CA_CERT || "/etc/ssl/certs/ca-certificates.crt",
          ),
          key: process.env.DB_CLIENT_KEY
            ? fs.readFileSync(process.env.DB_CLIENT_KEY)
            : undefined,
          cert: process.env.DB_CLIENT_CERT
            ? fs.readFileSync(process.env.DB_CLIENT_CERT)
            : undefined,
        }
      : false,
});
    ...
    contentSecurityPolicy: {
      directives: {
        ...
        upgradeInsecureRequests:
          process.env.NODE_ENV === "production" ? [] : ["'strict'"],
      },
    },
    strictTransportSecurity: {
...
const handleValidationErrors = (req, res, next) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    logger.security("Validation errors", {
      errors: errors.array(),
      path: req.path,
    });
...
module.exports = {
  ...
  publicRateLimit: createRateLimiter(
    100,
    60 * 1000,
    "Too many public requests",
  ),
