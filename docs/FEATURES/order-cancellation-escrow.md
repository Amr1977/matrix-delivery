...
if (customer.balance < requiredBalance) {
  throw new Error(
    `Need ${requiredBalance} EGP. Your balance: ${customer.balance}`,
  );
}
...
if (customer.available_balance < requiredBalance) {
  throw new Error(
    `COD orders require ${requiredBalance} EGP. Your balance: ${customer.available_balance}`,
  );
}
```
...
if (order.cod_paid_by_driver) {
  ...
  await balanceService.forfeitHold(
    customerId,
    orderId,
    reimbursement + distanceCompensation,
    driverId,
  );
}
```
